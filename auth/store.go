...
func (s *Store) CleanByRuntimeStatus(ctx context.Context, targetStatus string) int {
	...
	for _, acc := range accounts {
		...
		if s.db != nil {
			if err := s.db.DeleteAccountHard(ctx, acc.DBID); err != nil {
...
func (s *Store) CleanExpiredAccounts(ctx context.Context, maxAge time.Duration) int {
	...
	// 2. 批量物理删除数据库账号
	if s.db != nil {
		if err := s.db.BatchDeleteAccountsHard(ctx, expiredIDs); err != nil {
			log.Printf("过期清理: 批量删除数据库失败: %v，回退逐条处理", err)
...
func (s *Store) cleanExpiredFallback(ctx context.Context, ids []int64) int {
	cleaned := 0
	for _, id := range ids {
		if err := s.db.DeleteAccountHard(ctx, id); err != nil {
